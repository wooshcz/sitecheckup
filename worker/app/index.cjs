...
function run_lighthouse(url, job_id, opts, config = null) {
    db_client.query(
        "UPDATE public.jobs SET status='STARTED' WHERE job_id=$1;",
        [job_id],
    );
    ...
    chromeLauncher
        ...
        .then((chrome) => {
            ...
            lighthouse(url, opts, config).then((results) => {
                chrome.kill();
                db_client.query(
                    "UPDATE public.jobs SET status='FINISHED', results=$1 WHERE job_id=$2;",
                    [results.lhr, job_id],
                );
                console.log(
                    `${Date().toString()}: Job ${job_id} finished for ${url}`,
                );
                job_running = false;
            });
        });
...
function run_testssl(hostname, job_id, opts) {
    ...
    console.log(`${Date().toString()}: Job ${job_id} started`);
    const testSslCommand = "./testssl.sh --logfile " + job_id +
        ".log --jsonfile " + job_id + ".json " + opts.join(" ") + " " +
        hostname;
    exec(testSslCommand + " || true").then(() => {
        ...
        console.log(
            `${Date().toString()}: Job ${job_id} finished for ${hostname}`,
        );
        job_running = false;
    });
}
...
function run_linkchecker(url, job_id, opts) {
    ...
    exec(linkCheckerCommand + " || true").then((output) => {
        ...
        db_client.query(
            "UPDATE public.jobs SET status='FINISHED', results=$1 WHERE job_id=$2;",
            [JSON.stringify(xml_parsed.linkchecker), job_id],
        );
        ...
        try {
            ...
            try {
                if (params.type === "lighthouse") {
                    ...
                    db_client.query(
                        "UPDATE public.jobs SET status='NOT_SUPPORTED' WHERE job_id=$1;",
                        [params.job_id],
                    );
                    console.log(
                        `${Date().toString()}: Job ${params.job_id} has unsupported type`,
                    );
                }
            } catch (error) {
                db_client.query(
                    "UPDATE public.jobs SET status='ERROR' WHERE job_id=$1;",
                    [params.job_id],
                );
            ...
            console.log(
                `${Date().toString()}: ${error.input} is not a valid url`,
            );
            job_running = false;
        }
    }
...
async function main() {
    while (true) {
        if (job_running === false) {
            const queryResponse = await db_client.query(
                "SELECT * FROM public.jobs WHERE status='CREATED' ORDER BY created ASC;",
            );
